define([
	...
	var dom = {
		...
		hasClass: function hasClass(node, name) {
			//console.log(node, name, this.get(node));
			node = this.get(node);
		...
		/*
		 * Adds click- or switch-functionality plus associated function-calls
		 * @param {Object || String} node The element to be added with functionality
		 * @param {Function} on The function the be called when switch is active
		 * @param {Function} opt_off The function to be called when switch is inactive
		 */
		button: function button(node, btnOn, optBtnOff) {
			var that = this;
			...
			// on-off switch
				...
				node.onclick = function() {
					if (that.hasClass(node, "text-shadow-default")) {
						optBtnOff();
						that.removeClass(node, "text-shadow-default");
						that.removeClass(node, "btn-on");
						that.addClass(node, "btn-off");
					} else {
						btnOn();
						that.addClass(node, "text-shadow-default");
						that.removeClass(node, "btn-off");
						that.addClass(node, "btn-on");
					}
				};
			}
		},
		...
		/*
		 * Adds a transition-end-event-listener
		 */
